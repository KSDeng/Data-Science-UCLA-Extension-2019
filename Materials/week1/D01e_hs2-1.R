#####################################################################################################
# D01c. Data Cleaning, Processing, and Management
# Project for Anderson Joshoson Johnson Head Start Program: The individual Head Start Program
...
pre1 <- subset(pre, agency=="Bennington County Head Start")
post1 <- subset(post, agency=="Bennington County Head Start")

tpr.age=as.data.frame(table(pre1$age))
tpr.age$Perc <- tpr.age$Freq/sum(tpr.age$Freq)*100
tpo.age=as.data.frame(table(post1$age))
...
pre1$race1 <- gsub("Other.*","Other",pre1$race)
post1$race1 <- gsub("Other.*","Other",post1$race)

tpr.race=as.data.frame(table(pre1$race1))
tpr.race$Perc <- tpr.race$Freq/sum(tpr.race$Freq)*100
tpo.race=as.data.frame(table(post1$race1))
...
tpr.insurance=as.data.frame(table(pre1$insurance))
tpr.insurance$Perc <- tpr.insurance$Freq/sum(tpr.insurance$Freq)*100
tpo.insurance=as.data.frame(table(post1$insurance))
...
tpr.info.book=as.data.frame(table(pre1$info.book))
tpr.info.book$Perc <- tpr.info.book$Freq/sum(tpr.info.book$Freq)*100
tpo.info.book=as.data.frame(table(post1$info.book))
...
tpr.info.internet=as.data.frame(table(pre1$info.internet))
tpr.info.internet$Perc <- tpr.info.internet$Freq/sum(tpr.info.internet$Freq)*100
tpo.info.internet=as.data.frame(table(post1$info.internet))
...
tpr.info.tv=as.data.frame(table(pre1$info.tv))
tpr.info.tv$Perc <- tpr.info.tv$Freq/sum(tpr.info.tv$Freq)*100
tpo.info.tv=as.data.frame(table(post1$info.tv))
...
tpr.info.family=as.data.frame(table(pre1$info.family))
tpr.info.family$Perc <- tpr.info.family$Freq/sum(tpr.info.family$Freq)*100
tpo.info.family=as.data.frame(table(post1$info.family))
...
tpr.info.doctor=as.data.frame(table(pre1$info.doctor))
tpr.info.doctor$Perc <- tpr.info.doctor$Freq/sum(tpr.info.doctor$Freq)*100
tpo.info.doctor=as.data.frame(table(post1$info.doctor))
...
tpr.info.know=as.data.frame(table(pre1$info.know))
tpr.info.know$Perc <- tpr.info.know$Freq/sum(tpr.info.know$Freq)*100
tpo.info.know=as.data.frame(table(post1$info.know))
...
tpr.do.fever=as.data.frame(table(pre1$do.fever))
tpr.do.fever$Perc <- tpr.do.fever$Freq/sum(tpr.do.fever$Freq)*100
tpo.do.fever=as.data.frame(table(post1$do.fever))
...
tpr.do.vomit=as.data.frame(table(pre1$do.vomit))
tpr.do.vomit$Perc <- tpr.do.vomit$Freq/sum(tpr.do.vomit$Freq)*100
tpo.do.vomit=as.data.frame(table(post1$do.vomit))
...
tpr.do.earche=as.data.frame(table(pre1$do.earche))
tpr.do.earche$Perc <- tpr.do.earche$Freq/sum(tpr.do.earche$Freq)*100
tpo.do.earche=as.data.frame(table(post1$do.earche))
...
tpr.do.cough=as.data.frame(table(pre1$do.cough))
tpr.do.cough$Perc <- tpr.do.cough$Freq/sum(tpr.do.cough$Freq)*100
tpo.do.cough=as.data.frame(table(post1$do.cough))
...
tpr.sick.where=as.data.frame(table(pre1$sick.where))
tpr.sick.where$Perc <- tpr.sick.where$Freq/sum(tpr.sick.where$Freq)*100
tpo.sick.where=as.data.frame(table(post1$sick.where))
...
tpo.bettercare=as.data.frame(table(post1$bettercare))
...
hs2.benn=rbind(NA,NA,NA,NA,t.age, NA,NA,NA,t.race,NA,NA,NA,t.insurance,NA,NA,NA,t.info,NA,NA,NA,NA,t.do.fever,
                 NA,NA,NA,t.do.vomit,NA,NA,NA,t.do.earche,NA,NA,NA,t.do.cough,NA,NA,NA,t.sick.where,
                 NA,NA,NA,t.bettercare)

hs3.benn <- sapply(hs2.benn, as.character)
hs3.benn[is.na(hs3.benn)] <- ""
write.csv(hs3.benn,"~/zip08/2019 Q3 Summer_XData/Output/hs3.benn.csv", row.names=F)
